...
fn calculate_fees(
    ...
    if protocol_fee_rate > 0 {
        // Calculate the delta based on the fee amount and the protocol fee rate
        let delta = ((global_fee as u128) * (protocol_fee_rate as u128) / PROTOCOL_FEE_RATE_MUL_VALUE)
            .try_into()
            .unwrap();

        // Subtract the delta from the global fee
        global_fee -= delta;

        // Add the delta to the next protocol fee
        next_protocol_fee = next_protocol_fee.wrapping_add(delta);
    }

    // Calculate the fee growth global input
    if curr_liquidity > 0 {
